...
function normalizeOptions(value: unknown): AnswerOption[] {
  ...
  return value.flatMap((option, index) => {
    ...
    const content = asString(option.content);
    if (!content) return [];
    return [{ id: asString(option.id) || String(index), content }];
  });
}
...
export function normalizeQuestion(value: unknown): SatQuestion | null {
  ...
  const answerOptions = normalizeOptions(value.answerOptions ?? content.answerOptions);
  const rawType = asString(value.type ?? content.type).toLowerCase();
  const type = rawType === "spr" ? "spr" : "mcq";
  const questionId = asString(value.questionId ?? content.questionId);
  const stem = asString(value.stem ?? content.stem ?? content.prompt);
  const correctAnswer = normalizeAnswers(value.correct_answer ?? content.correct_answer);

  if (!questionId || !stem || correctAnswer.length === 0) return null;
  if (type === "mcq" && answerOptions.length === 0) return null;

  const rawStimulus = value.stimulus ?? content.stimulus;
  const scoreBand = Math.min(7, Math.max(1, asNumber(value.score_band_range_cd ?? content.score_band_range_cd, 3)));
  
  let difficulty = asString(value.difficulty ?? content.difficulty);
  if (!difficulty) {
  ...
  const rawModule = asString(value.module ?? content.module).toLowerCase();
  const module = rawModule.includes("math") ? "math" : "reading";

  return {
    questionId,
    updateDate: asNumber(value.updateDate),
    primary_class_cd: asString(value.primary_class_cd ?? content.primary_class_cd),
    skill_cd: asString(value.skill_cd ?? content.skill_cd),
    ...
    rationale: asString(value.rationale ?? content.rationale),
  };
}
...
export async function parseAndIngestJSON(file: File): Promise<number> {
  ...
  const candidateQuestions = Array.isArray(json)
    ? json
